...
const {height: SCREEN_HEIGHT, width: SCREEN_WIDTH} = RNDimensions.get("screen");

export default function ImageItem({source}: IProps) {
	...
	const setToCenter = () => {
		...
		positionX.value = withTiming(SCREEN_WIDTH / 2 - dimensions.dimensions.width / 2, {duration: 200});
		positionY.value = withTiming(SCREEN_HEIGHT / 2 - dimensions.dimensions.height / 2, {duration: 200});
	}

	const onPinchUpdate = (event: GestureUpdateEvent<PinchGestureHandlerEventPayload>) => {
		"worklet";
	...
	const onDoubleTap = (event: GestureUpdateEvent<TapGestureHandlerEventPayload>) => {
		...
		zoomScale.value = withTiming(zoomScale.value === 1 ? 2 : 1, {duration: 300});
	}

	const onPanUpdate = (event: GestureUpdateEvent<PanGestureHandlerEventPayload>) => {
		"worklet";

		// We move the position based on the pan. We also have to divide this by the zoom scale.
		positionX.value = positionX.value + (event.translationX - lastTransitionX.value) / zoomScale.value;
		positionY.value = positionY.value + (event.translationY - lastTransitionY.value)/ zoomScale.value;
	...
	const onPanEnd = (event:  GestureStateChangeEvent<PanGestureHandlerEventPayload>) => {
		"worklet";
	...
	return (
		<>
			<Pressable onPress={onRequestOpenOrClose} ref={nonViewerRef} style={{opacity: expanded ? 0 : 1}}>
				<FastImage source={source} style={dimensions.dimensions} onLoad={onLoad} />
			</Pressable>
